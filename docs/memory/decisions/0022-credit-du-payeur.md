...
- **Concurrence** : la transaction prend deux verrous `pg_advisory_xact_lock`, la personne puis la facture, avant de lire le crédit et le reste dû. Deux imputations simultanées ne peuvent ni dépenser deux fois le même crédit, ni surpayer la facture.
  ...
  - **Qui prend le verrou de la facture** :
  ...
  - **Limite** : l'encaissement carte (webhook Stripe) ne prend pas ce verrou. Une annulation ne le voit que s'il est déjà commité quand elle relit la facture ou, pour une annulation boutique, quand elle l'écrit. S'il arrive après, le paiement reste sur une facture annulée, comme avant.
