...
## 2026-05-04 14:55 (6e session — fin de journée Phase 1+2 multi-tenant complet)
- Score : 24 (formule v3 — 5 commits dédup, 13 déjà loggés exclus)
- Commits scannés (SHA) : a2ceb45, 3696512, cf1c009, d22ab29, 34af35e
- Créés : `pitfalls/brevo-smtp-user-is-not-account-email.md`, `pitfalls/brevo-sender-domain-must-be-authenticated.md`, `pitfalls/powershell-auto-converts-urls-to-markdown.md`
- Refusés : workflow forgot password (self-evident dans code), pitfall Gmail/DMARC séparé (intégré comme note dans brevo-sender-domain)
- Note : grosse session debug mail Brevo en fin de journée (1h30 cumulée). Les 3 pitfalls couvrent le triplet : auth SMTP / sender authentifié / corruption clipboard. Devrait épargner une heure de debug similaire la prochaine fois qu'on touche au mail.
