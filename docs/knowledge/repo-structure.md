...
│   ├── landing/          Landing marketing Next.js (port 5176 dev+prod)
...
├── docs/
│   ├── architecture/     Carte du projet + guide Graphify
│   ├── knowledge/        ← TU ES ICI
│   ├── runbooks/         Procédures multi-étapes
│   └── memory/           Pitfalls, décisions, workflows appris
├── .github/workflows/    CI/CD (release-please, deploy, deploy-staging, typecheck, validate-memory)
├── .claude/skills/       Skills Claude du projet (versionnés)
...
- **`.claude/skills/`** est versionné — les skills `/deploy`, `/release`,
  `/restart`, etc. sont partagés avec le repo (cf. tableau des skills dans
  CLAUDE.md).
- **`docs/superpowers/`** contient les specs et plans d'implémentation
  internes du projet (`specs/`, `plans/`, `roadmap/`).
