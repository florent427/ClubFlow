...
// Outillage partagé avec la lecture des relevés bancaires (lot 2 du
// rapprochement) : PDF → texte natif et pages images, choix du modèle
// vision, prétraitement et tuilage des images.
import {
  bufferToDataUrl,
  errorMessage,
  extractPdfText,
...
- paymentReference : retiens la référence si l'une des 2 sources l'a trouvée
  ...
  /** PDF → pages PNG (cf. `ocr-shared.rasterizePdf`). */
  private async rasterizePdf(
    buf: Buffer,
  ): Promise<Array<{ buffer: Buffer; mimeType: string }>> {
    return rasterizePdf(buf, this.logger);
  }
  ...
  /** Prétraitement image (cf. `ocr-shared.preprocessImage`). */
  private async preprocessImage(
    buf: Buffer,
    mimeType: string,
  ): Promise<{ buffer: Buffer; mimeType: string }> {
    return preprocessImage(buf, mimeType, this.logger);
  }

  /** Tuilage des images très hautes (cf. `ocr-shared.tileTallImage`). */
  private async tileTallImage(
    buf: Buffer,
    mimeType: string,
  ): Promise<Array<{ buffer: Buffer; mimeType: string }>> {
    return tileTallImage(buf, mimeType, this.logger);
  }
}
