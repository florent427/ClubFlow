...
| PR de release fusionnée mais aucun tag ni release | `GITHUB_TOKEN` ne réveille aucun workflow — arrive à **chaque** promotion | [pitfall](docs/memory/pitfalls/github-token-ne-retrigger-pas.md) |
...
| « Validez un domaine d'envoi… » alors que le domaine est `VERIFIED` | le `purpose` ne couvre pas cet usage | [pitfall](docs/memory/pitfalls/domaine-envoi-role-non-modifiable.md) |
